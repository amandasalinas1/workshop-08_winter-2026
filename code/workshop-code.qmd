...
[deployed html url](https://amandasalinas1.github.io/workshop-08_winter-2026/code/workshop-code.html)
...
# start with the original data frame to make new data frame
drought_exp_clean <- drought_exp |> 
  # clean up names with underscores and lowercases
  clean_names() |> 
  # changed the species code to the scientific name in a new column
  ...
  mutate(water_treatment = as_factor(water_treatment),
         water_treatment = fct_relevel(water_treatment,
                                       "Drought stressed",
                                       "Well watered")) |> 
  # selects only the four columns listed
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = "Specific Leaf Area cm\U00B2/(g)",
       y = "Total mass (g)",
       title = "SLA increases and total mass increases") 
```
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = "Water Treatment",
       y = "Total mass (g)",
       title = "Well watered plants tend to have a higher mass") +
...
**Figure 4: Plant mass (g) differs across species and treatments.** Open circles represent individuals in each treatment (Red: Drought stressed, Blue: well watered treatment). Panels depict each different species. Opaque points represent model predictions for plant mass, and bars represent the 95% confidence intervals of model predictions. Data source: Valliere, Justin; Zhang, Jacqueline; Sharifi, M.; Rundel, Philip (2019). Data from: Can we condition native plants to increase drought tolerance and improve restoration success? [Dataset]. Dryad. [https://doi.org/10.5061/dryad.v0861f7](https://doi.org/10.5061/dryad.v0861f7)
...
#### Goal of analysis
We used linear models to determine the predictive relationship between specific leaf area (cm^2^/g), treatment, and species on plant mass (g). 

#### Model selection
We chose the simplest model that explained the most variation in plant mass using Akaike's Information Criterion (AIC)

#### Model diagnostics
We evaluated model assumptions for the simplest model chosen using AIC for normally distributed errors (residuals) using QQ plots and homoscedastic errors (residuals) using Residuals vs Fitted and Scale-Location plots.

### Results
We found that the simplest model describing variation in plant mass included species and treatment (linear model: F(7, 62) = 27.08, p < 0.001, $\alpha$ = 0.05, adjusted R^2^ = 0.73)
